...
mcmc.2side.SCRIPP <-
  function(data,niter=2400,nburn=1200, nthin=5, M = 200, cellArea=cellArea,inits=inits,swap=10,swap.tol=1,proppars=list(lam01=0.05,lam02=0.05,sigma=0.1,sx=0.2,sy=0.2),keepACs=TRUE,Rcpp=TRUE){
    if(Rcpp==TRUE){
      out2=mcmc.2side.SCRIPP.Rcpp(data,niter=niter,nburn=nburn, nthin=nthin, M = M, cellArea=cellArea,inits=inits,swap=swap,swap.tol=swap.tol,proppars=proppars,keepACs=keepACs)
    }else{
      out2=mcmc.2side.SCRIPP.R(data,niter=niter,nburn=nburn, nthin=nthin, M = M, cellArea=cellArea,inits=inits,swap=swap,swap.tol=swap.tol,proppars=proppars,keepACs=keepACs)
    }
    if(keepACs==TRUE){
