  ...
  function(data,niter=2400,nburn=1200, nthin=5, M = 200, inits=inits,swap=10,swap.tol=1,proppars=list(lam01=0.05,lam02=0.05,sigma=0.1,sx=0.2,sy=0.2),keepACs=FALSE){
    ...
    if(uplam02){
      ...
      if(!is.na(inits$lam02)){
        warning("User specified init for lam02 with no double traps. Setting to 0.")
      }
    }
    ...
    #Run MCMC
    store=SCRRcpp::MCMCtf( lam01, lam02,  sigma,y.both,  y.left,  y.right,  z,  X, tf,K,D, Nfixed,known.vector,
