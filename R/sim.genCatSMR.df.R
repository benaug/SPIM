...
#' without telemetry data. This version allows detection function parameters to vary by levels of 
#' one categorical covariate
#' @param N abundance
#' @param lam0.mark a vector of detection function baseline detection rates for the marking process 
#' the same length as the first list element in IDcovs. Converted to p0, the baseline detection 
#' probability if the obstype="bernoulli".
#' @param lam0.sight a vector of detection function baseline detection rates for the sighting process 
#' the same length as the first list element in IDcovs. Converted to p0, the baseline detection 
#' probability if the obstype="bernoulli".
...
sim.genCatSMR.df <-
  function(N=50,lam0.mark=0.25,lam0.sight=0.25,sigma=0.50,K1=10,K2=10,Korder=NA,X1=X1,X2=X2,buff=3,obstype="bernoulli",ncat=ncat,
    ...
    #Variable df stuff
    if(length(lam0.mark)!=length(lam0.sight)&length(lam0.mark)!=length(sigma)){
