...
#' @param sigma_d the detection function spatial scale parameter assumed to be the same for both capture
#' processes. This could be relaxed.
#' @param sigma_p the spatial scale parameter for activity center relocation between capture processes.
...
#' @export
sim.genCatSMR.move <-
  function(N=50,lam0.mark=0.25,lam0.sight=0.25,sigma_d=0.50,sigma_p=0.50,K1=10,
           K2=10,X1=X1,X2=X2,buff=3,obstype="bernoulli",ncat=ncat,
    ...
    X2=as.matrix(X2)
    Xall=rbind(X1,X2)
    xlim=c(min(Xall[,1])-buff,max(Xall[,1])+buff)
    ylim=c(min(Xall[,2])-buff,max(Xall[,2])+buff)
    s1<- cbind(runif(N, xlim[1],xlim[2]), runif(N,ylim[1],ylim[2]))
    s2=matrix(NA,ncol=2,nrow=N)
    for(i in 1:N){
      s2[i,]=c(rnorm(1,s1[i,1],sigma_p),rnorm(1,s1[i,2],sigma_p))
      inside=s2[i,1]>xlim[1]&s2[i,1]<xlim[2]&s2[i,2]>ylim[1]&s2[i,2]<ylim[2]
      while(inside==FALSE){
        s2[i,]=c(rnorm(1,s1[i,1],sigma_p),rnorm(1,s1[i,2],sigma_p))
        inside=s2[i,1]>xlim[1]&s2[i,1]<xlim[2]&s2[i,2]>ylim[1]&s2[i,2]<ylim[2]
      }
    ...
    s1full=s1
    s2full=s2
    #turning off Korder. Does not make sense for activity center to move between processes
    #if they overlap
    # if(is.na(Korder[1])){
    ...
    if(tlocs>0){
      ...
      for(i in 1:n.marked){
        for(j in 1:tlocs){
          locs[i,j,]=c(rnorm(1,s2[IDmarked[i],1],sigma_d),rnorm(1,s2[IDmarked[i],2],sigma_d))
        }
      }
    }else{
      locs=NA
